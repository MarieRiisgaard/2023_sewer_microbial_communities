...
# see <- sewer_data_list[[2]] %>% 
#   group_by(SampleContent) %>% 
#   filter(SampleContent == "biofilm_dry")
#   summarise(n = n())

  
  # sewer_data_list[[1]] %>% 
  #   amp_subset_samples(SampleContent == "biofilm_dry") %>% 
  #   amp_heatmap(tax_aggregate = "Genus", tax_show = 60, round = 3, group_by = "SampleDate", normalise = T)
  # 
  # 
  # sewer_data_list[[1]] %>% 
  #   amp_subset_samples(SampleContent == "biofilm_dry") %>% 
  #   amp_subset_taxa(tax_vector = Nitrifiers, normalise = T) %>% 
  #   amp_heatmap(tax_aggregate = "Genus", tax_show = 60, round = 2, group_by = "SampleDate", normalise = F) + 
  #   sewer_data_list[[1]] %>% 
  #   amp_subset_samples(SampleContent == "biofilm_dry") %>% 
  #   amp_subset_taxa(tax_vector = Nitrifiers, normalise = T) %>% 
  #   amp_heatmap(tax_aggregate = "Species", tax_show = 60, round = 2, group_by = "SampleDate", normalise = F, tax_add = "Genus")
...
SampleContent3_colors<- c("#1e8449","#b03a2e","#21618C", "#ffcd80","#e69a3a", "#e65100", "#795548")


# Rarefied data
alpha_diversity_rarefied <- data %>% 
  amp_alpha_diversity(rarefy = 7322) %>% 
  left_join(data_long %>% select(SampleID, SampleContent3) %>%rename("SampleID2" = "SampleID")) %>% 
  mutate(SampleContent3 = 
           factor(SampleContent3, 
            levels = c("Biofilm (g.)", "Sediment", "Biofilm (e.p.)", "SWW\n(g.)","SWW\n(e.p.)" ,"IWW","AS"),
            labels = c("Biofilm (g.)", "Sediment", "Biofilm (e.p.)", "SWW (g.)","SWW\n(e.p.)" ,"IWW","AS")
...
source(paste0(WD, "scripts/functions/growing_dying_bar_plot_new_version.R"))
...
# data %>% 
#   amp_subset_samples(SampleContent2 == "AS") %>% 
#   amp_heatmap(tax_aggregate = "Genus", tax_show = 50)
...
biofilm_g_new_detected <- 
    ...
    unnest(samples) %>%
    distinct(SampleID, Species, over_limit, n_samples) %>% 
    group_by(Species) %>% 
    mutate(n_obs_tax_over_zero = n()) %>% ungroup() %>% 
    group_by(Species, over_limit) %>%
    reframe(
      n_obs_tax = n(), 
      n_obs_tax_over_zero,
      is_core = ifelse(n_obs_tax >= 0.8*n_samples, T, F), 
      core_type = case_when(n_obs_tax >= 0.8*n_samples & over_limit ~ "strict core",
                            n_obs_tax >= 0.5*n_samples & over_limit ~ "general core",
                            n_obs_tax >= 0.2*n_samples & over_limit & 0.2*n_samples > 1 ~ "loose core", 
                            (n_obs_tax < 0.2*n_samples | !over_limit) & n_obs_tax_over_zero > 1  ~ "detected", 
                            (n_obs_tax < 0.2*n_samples | !over_limit) & n_obs_tax_over_zero == 1  ~ "not detected", 
                            .default = "missing"),
            n_samples)  %>% 
  distinct() %>% 
   mutate(
...
best_core_group <- rbind(
  biofilm_g_new_detected, biofilm_ep, sediment
  ) %>% 
  select(-over_limit, -n_obs_tax_over_zero, -core_type_f) %>% 
  arrange(Species) %>% 
  mutate(
    core_type_f = factor(core_type, 
                         levels = c("strict core", "general core", "loose core", 
  ...
  select(-core_type_f) %>% 
  arrange(Species) %>% 
  select(-n_obs_tax, -is_core, -n_samples) %>% 
...
plot <- data_long %>%
  ...
  mutate(year = lubridate::year(SampleDate), 
         ...
         SampleContent2 = fct_recode(SampleContent2, "Bio (e.p.)"="Biofilm (e.p.)"),
                  SampleContent2 = fct_recode(SampleContent2, "Bio (g.)"="Biofilm (g.)"),
  ...
  geom_point(aes(color = SampleContent2), position = position_dodge(0.8), size = 2) + 
    scale_color_manual(values = SampleContent2_colors) +
  ...
  theme_bw() +
  guides(color = guide_legend(title = "Habitat", nrow = 1)) +
  facet_nested(SampleContent2~x, #strip = strip_background, 
               scales = "free_y", space = "free"
               ) + 
  theme(#axis.text.x = element_text(angle = 0, vjust = 0.5, hjust = 0.5, size = 14),
        axis.text.y = element_text(size = 14, color="grey10"),
        ...
        legend.title = element_text(size = 14, color = "black", margin = margin(b = unit(5,"mm"))), 
        panel.spacing=unit(2,"mm"),
        panel.background = element_rect(fill = "white"),
      strip.background.x=element_rect(color="grey90", fill="grey90"),
      strip.background.y = element_rect(color="grey90", fill="grey90"),
...
ggsave(plot = plot, paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".png"), width = 10, height = 6)
...
plot <- alpha_diversity_rarefied %>% 
  ...
  theme_bw() + 
  guides(color = guide_legend(override.aes = list(size = 4), nrow = 1, title = "Habitat")) +
  theme(#axis.text.x = element_text(angle = 0, vjust = 0.5, hjust = 0.5, size = 14),
        axis.title.x = element_blank(), 
...
plot <- data %>% 
  ...
  guides(color = guide_legend(ncol = 1, title = "Habitat"),
         fill = guide_legend(ncol = 1, title = "Habitat")
         )
...
tax_level_name <- ifelse(str_detect(data_long %>% sample_n(1) %>% unnest(samples) %>% 
                                        filter(str_detect(Species,"nclassified", T) |
                                                 str_detect(Species, "midas", T)) %>% 
                                        select(Species) %>% unlist(), "g__"),
                           "genera", 
                           "species")
  ...
  see2 <- data_long %>% 
    mutate(samples = 
             map(.x = samples, ~
                   ungroup(.) %>% 
                   mutate(uncla = 
                            case_when(str_detect(Species, "unclassified") & tax_level_name == "species" & 
                                        !OTU %in% unlist(usearch_results$query) ~ "Unclassified ASVs", 
                                      str_detect(Species, "unclassified") & tax_level_name == "genera" ~ "Unclassified ASVs", 
                                      str_detect(Species, "unclassified") & tax_level_name == "species" &
                                        OTU %in% unlist(usearch_results$query) ~ "Multiple species hits in the MiDAS database",
                                      !str_detect(Species, "unclassified")  & tax_level_name == "species" ~ "Species level classification",
                                      !str_detect(Species, "unclassified")  & tax_level_name == "genera" ~ "Genus level classification",
                                      .default = "missing"),
                    ) %>% 
                   distinct(across(c(Species, rel_abun_species, uncla))) %>% 
                   filter(rel_abun_species != 0) %>% 
                   group_by(uncla) %>% 
                   summarise(sum_abun = sum(rel_abun_species), n_species = n(), .groups = "drop")
             )) %>% 
    unnest(samples) %>% 
  ...
  # group_by(SampleContent2, System,names) %>%
  #   mutate(uncla = factor(uncla, levels = c("Unclassified ASVs", "Species level classification", "Multiple species hits in the MiDAS database"))) %>%
  #   arrange(uncla) %>% 
  ...
  plot <- see2 %>%
    ...
    arrange(desc(uncla)) %>% 
      mutate(
        y_axis = values/sum(values),
        p = cumsum(y_axis),
      # p = case_when(uncla == "Unclassified ASVs" ~ 0.05, 
      # str_detect(uncla, "level classification") ~ 0.5,
      # .default = 0.87),
      y = "",
       ) %>% 
    ungroup() %>% 
    ggplot(aes(fill = uncla, x = names, y = y_axis)) + 
    geom_col(position="fill", width = 0.98, #stat="identity"
             ) + 
    ggh4x::facet_nested(y~System + SampleContent2) +
    geom_text(aes(label = ifelse(names == "mean", 
                                 paste0(round(values, 0), "%"), 
                                 paste0(round(values, 0))       
    ), y = p - y_axis* 0.3), size = 8) + 
...
PCOA_OTU <- data_long %>% 
  ...
  scale_fill_manual(values = SampleContent2_colors) +
  theme_bw() + 
  guides(fill = guide_legend(title = "Habitat   ", nrow = 1),
         color = guide_legend(title = "Habitat   ", , nrow = 1)) +
  theme(#legend.title = element_blank(), 
        legend.position = "bottom",
        #axis.text.y = element_text(size = 12, color = "black"),
...
plot1 <- plot + 
  ...
  theme(legend.position = "bottom",
        ...
        plot.tag = element_text(size = 30, color = "black"), 
...
plot <- 
  ...
  theme(axis.text.y = element_markdown(size = 12, color = "black"),
        legend.text = element_text(size = 12, color = "black", margin = margin(l = unit(5,"mm"))),
        legend.title = element_text(vjust = 0.8, size = 16, margin = margin(r = unit(15,"mm"))),
...
## OLD: Figure S7: Growth bar plot each SWW site 
...
## Figure S11: Most abundant species across all
...
top_each_samplecontent %>% distinct(Species)
# 
top_each_samplecontent_groups <- top_each_samplecontent %>%
  group_by(Species) %>% 
  summarise(SampleEnvironment_obs = paste0(unique(SampleEnvironment), collapse = ";"))
...
plot_name = "S11_heatmap_top10_abundant_species_in_all"
...
## Figure 3: PCoA of sewer envionments (Sample types)
...
```{r}
  ...
  colors <- c("#1E3F66", "grey75")   #"#4D4D4D", "#A6A6A6", 

  #colors <- c("#A7C7E7","#D6604D", "#F4A582", "#FDDBC7", "white","grey90", "grey60") ## Added

  
  # DF ready for growth 
  
  gut_ASV <- data_long %>% sample_n(1) %>% unnest(samples) %>%
    select(OTU, Species) %>%
    filter(OTU %in% unlist(gut_OTU$OTU)) %>%
    distinct(OTU) %>% unlist()
  
  #data_long %>% distinct(SampleContent3)
  
  source_df <- data_long %>% sample_n(1) %>% unnest(samples) %>% 
    select(OTU, Species) %>% 
    mutate(
      #core_group = NA, 
      ...
      core_group = if_else(OTU %in% gut_ASV, "Gut species", "Other source"),
      core_group = factor(core_group, 
                          levels = c("Gut species", 
                                     #"strict core","general core", "loose core", 
                                     #"CRAT", ## Added
                                     "Other source"),
  ...
  tidy_all_influents <- data_long %>%
    ...
    unnest(samples) %>% 
    group_by(SampleContent3, System, origin_groups) %>% 
    summarise(
  ...
  barplot_relative_abundance <- 
    ...
    mutate(
      ...
      newy=cumsum(values), 
      #group_name = 
      SampleContent3 = factor(SampleContent3, 
                          levels = c(#"Biofilm (g.)",  "Sediment", "Biofilm (e.p.)", 
                                     "SWW\n(g.)", "SWW\n(e.p.)", # "SWW", 
                                     "IWW","AS"),
                          labels = c(#"Biofilm\n(g.)",  "Sedi-\nment", "Biofilm\n(e.p.)",
                            "SWW (g.)", "SWW (e.p.)", # "SWW", 
    ...
    ################## Pattern version
    ggplot(aes(fill=origin_groups, y=values, x=SampleContent3)) + 
    ...
    geom_label(aes(x = SampleContent3, y = newy-(values*0.5),
                   color = if_else(str_detect(origin_groups, "Gut"), "white", "black"),
                   label = label, 
    ), 
    ...
    theme(legend.text = element_markdown(size = 14), 
          ...
          axis.title.y = element_text(size = 16, color  = "black"),# element_blank(), #element_text(size = 16), 
          axis.title.x = element_blank(), 
          text = element_text(color  = "black"),
          legend.key = element_rect(linetype = 1), 
          strip.text.x = element_text(angle = 0, size = 14, vjust = 0.5, hjust = 0, 
    ...
    scale_x_discrete(expand = c(0,0)) + 
    ggh4x::facet_nested(v~System+SampleContent3, scales = "free_x", space = "free")
...
PCOA_genus <- data_long %>% 
  ...
  annotate("richtext",  x = -0.5, y = -0.47,
           label.colour = "white", 
           label = 
              "R<sup>2</sup> = 41%, p<0.001", hjust = 0, size = 5, 
           label.padding = unit(0.01, "mm")) +
  annotate("richtext", x = -0.5, y = -0.43, hjust = 0, size = 5,
           label.colour = "white",label.padding = unit(0.01, "mm"),
           label = "ADONIS: Habitat") 
...
plot_name = "F3_PCA_results_sewer_envionments_without_SWW"
...
plot <- 
  barplot_relative_abundance +
  plot_spacer() + PCOA_genus + plot_spacer() +
...
plot <- 
  ...
  barplot_relative_abundance + 
  plot_spacer() + 
  PCOA_genus + guides(fill = guide_legend(title = "Habitat  "),
                      color = guide_legend(title = "Habitat  ")) +
  plot_spacer() + 
  plot_annotation(tag_levels = "A") + 
  plot_layout(
    ...
    design = c(
      area(1, 1),
      area(2, 1, 3, 1),  # First plot spans 3 rows in the first column
      area(1, 2),        # Spacer in the second column
      area(2, 2),        # PCOA_genus in the second column
      area(3, 2)         # Spacer in the second column below PCOA_genus
    ),
    widths = c(0.8, 1),  # Make the first column wider
    heights = c(0.00000001, 1, 0.000001)  # Custom heights for rows
  ) & 
  theme(plot.tag = element_text(size = 30),
        legend.title = element_text(size = 16),
        legend.position = "bottom")

plot_name = "F3_gut_fractions_and_PCOA"
ggsave(plot = plot, paste0(OutputPath, "plots/1_results/", plot_name, ".png"), 
        width = 12, height = 8)
ggsave(plot = plot, paste0(OutputPath, "plots/1_results/", plot_name, ".pdf"), limitsize = F, useDingbats=FALSE, 
       width = 12, height = 8)
...
## Figure S8: PCoA of sewer envionments (Sample sites)
...
PCOA_genus <- data_long %>% 
  ...
  guides(fill = guide_legend(title = "Sample Site"),
         color = guide_legend(title = "Habitat", override.aes = list(shape = c(16,17))), 
...
plot_name = "S8_PCA_results_sewer_envionments_samplesite"
...
## Figure S9: Most abundant species across all sites - sewer microbiome
...
plot_name = "S9_heatmap_top30_abundant_species_in_sewer_microbiome"
...
## Figure 4: Stacked bar - Best core
...
colors <- c("#1E3F66", "#41729F", "#76A5C9", "#B0D0E8",  "grey90","grey60")   #"#4D4D4D", "#A6A6A6", 

gradient2 <- c("#A6A6A6", "#006400", "#2C804A", "#7AA36E")
    ...
    mutate(
      ...
      core_group = factor(core_group, 
                          levels = c("Gut species", 
                                     "strict core","general core", "loose core", 
                                     #"CRAT", ## Added
                                     "detected","Unknown source"),
                          labels = c("Gut species", 
                                     "Strict core","General core", "Loose core",
                                     # "CRAT", ## Added
                                     "Detected in the sewer microbiome","Unknown source"))
    ) %>% 
    distinct(core_group, OTU) %>% 
    rename("origin_groups" = "core_group")
  ...
  barplot_relative_abundance <- 
    ...
    geom_label(aes(x = group_name, y = newy-(values*0.5),
                   color = if_else(str_detect(origin_groups, "Gut") | str_detect(origin_groups, "trict"), "white", "black"),
                   label = label
    ), 
    #position = position_dodge(width = 0.3),
    label.padding = unit(0.1, "lines"),
    ...
    theme(legend.text = element_markdown(size = 16), 
        ...
        panel.spacing=unit(1,"mm"),
        axis.ticks.x = element_blank(), 
        legend.title = element_markdown(size = 16,  margin = margin(r = unit(15,"mm"))), 
        strip.background.x = element_rect(fill = "grey90"),
        strip.background.y = element_blank()
        ) +
    scale_y_continuous(expand = c(0.015,0), breaks = c(0,25,50,75,100), labels = c("0%","25%","50%","75%","100%")) +
...
plot_n_species <- stacked_bar_for_article(data_long_f = data_long, 
                        ...
                        facet_by = SampleContent3, 
                        sewer_core_species = core_species_all,
                        type_select = 'No. of species (transformed) [%]'
) 
...
plot_name = "F4_stacked_bar_best_core_0.01_AB"
ggsave(plot = plot, paste0(OutputPath, "plots/1_results/", plot_name, ".png"), 
       width = 14, height = 8)
ggsave(plot = plot, paste0(OutputPath, "plots/1_results/", plot_name, ".pdf"), limitsize = F,  useDingbats=FALSE,  
       width = 14, height = 8)
...
## Figure 5: Core groups with pattern 
...
```{r}
growth_and_group <- best_core_group %>% select(Species, best_core_group) %>% 
  rbind(gut_OTU %>% 
          mutate(best_core_group = "Gut bacteria, Disappearing") %>% 
          select(-OTU) %>% distinct(Species, best_core_group)) %>% 
  mutate(best_core_group = if_else(str_detect(best_core_group, " core"), "Sewer core", best_core_group)) %>% 
  full_join(.,data_long %>% sample_n(1) %>% unnest(samples) %>% distinct(OTU, Species)) %>% 
  mutate(best_core_group = if_else(is.na(best_core_group), "not detected", best_core_group)) %>% 
  distinct(OTU, Species, best_core_group) %>% 
  left_join(growth_df, by = c("OTU")) %>% 
  mutate(growth_AS = if_else(growth_AS %in% c("surviving", "variable"), "surviving/varible", growth_AS))


data <- 
  data_long %>% #sample_n(10) %>% 
   mutate(
     SampleContent2 = SampleContent3,
     samples = 
           map(.x = samples, ~
                 ungroup(.) %>% 
                 left_join(growth_and_group, by = join_by(Species, OTU)) %>% 
                 filter(rel_abun_ASV != 0) %>% 
                 group_by(growth_AS, best_core_group) %>% 
                 summarise(sum_growth = sum(rel_abun_ASV), 
                           n_species = n_distinct(Species),
                           .groups = "drop")
           )) %>% 
  unnest(samples) %>% #select(sum_growth,n_species, growth_AS, best_core_group, SampleID) %>% print(n=100)
  group_by(SampleContent2, growth_AS, best_core_group#, Configuration
           ) %>% 
...
df <- data %>%
  group_by(best_core_group, SampleContent2) %>%
  mutate(
    abun_best_core = sum(mean_abun), 
    species_best_core = sum(mean_n_species), 
    growth_AS = paste0(best_core_group, ";", growth_AS)
  ) %>%
  ungroup() %>% 
  pivot_longer(cols = c(best_core_group, growth_AS), 
               names_to = "names", values_to = "values") %>% 
  mutate(
    merge_abundances = if_else(names == "best_core_group", abun_best_core, mean_abun),
    merge_species = if_else(names == "best_core_group", species_best_core, mean_n_species),
    ) %>%
  select(-mean_abun, -mean_n_species, -species_best_core, -abun_best_core) %>% distinct()


ordered_values <- c(
  "Gut bacteria, Disappearing;unknown", 
  "Gut bacteria, Disappearing;disappearing", 
  "Gut bacteria, Disappearing;surviving/varible", 
  "Gut bacteria, Disappearing;growing", 
   "Sewer core;unknown", 
  "Sewer core;disappearing", 
  "Sewer core;surviving/varible", 
  "Sewer core;growing",
  "detected;unknown", 
  "detected;disappearing", 
  "detected;surviving/varible", 
  "detected;growing",
  "not detected;unknown", 
  "not detected;disappearing", 
  "not detected;surviving/varible", 
  "not detected;growing",
  "Gut bacteria, Disappearing", 
  "Sewer core", 
  "detected", 
  "not detected"
)
...
ordered_labels <- c(
  ...
  "not detected;growing",
  "Gut species (GS)", 
  "Unified sewer core (USC)", 
  "Detected in the sewer microbiome (DSM)", 
  "Unknown source (US)"
)

# col2 = c(col, col, col, col, "#A7C7E7", "#D6604D", "grey90", "grey60")
# col2 = c(col, col, col, col,"#5795D1", "#A7C7E7",  "grey90", "grey60")
# col2 = c(col, col, col, col,"#1E3F66", "#B0D0E8",  "grey90","grey60")   #"#4D4D4D", "#A6A6A6", 
col2 = c(col, col, col, col,"#1E3F66", "#76A5C9",  "grey90","grey60")   #"#4D4D4D", "#A6A6A6", 


plot_df <- df  %>% 
  mutate(
    values = factor(values, levels = ordered_values, labels = ordered_labels), 
    names = factor(names),
    width = if_else(names == "best_core_group", 0.5, 1.5),
    )  %>% 
    left_join(data_long %>% distinct(SampleContent3, System) %>% rename("SampleContent2" = "SampleContent3")) %>% 
  mutate(names=recode(names, 'best_core_group'='Source'),
         names=recode(names, 'growth_AS'='Growth\nfate in AS')) %>% 
  mutate(SampleContent2 = factor(SampleContent2, 
                          levels = c("Biofilm (g.)",  "Sediment", "Biofilm (e.p.)", "SWW\n(g.)", "SWW\n(e.p.)", # "SWW", 
                                     "IWW","AS"),
                          labels = c("Biofilm\n(g.)",  "Sedi-\nment", "Biofilm\n(e.p.)","SWW\n(g.)", "SWW\n(e.p.)", # "SWW", 
                                     "IWW","AS"))) 
...
label_df_groups <- 
  ...
  arrange(SampleContent2,values) %>% 
  group_by(SampleContent2, System) %>%
   mutate(
    mean_abun_sum = cumsum(merge_abundances)) 
...
line_data <- plot_df %>%
  ...
  group_by(SampleContent2, System) %>% 
  mutate(
    values = factor(values, levels = c("Gut bacteria, Disappearing",  "Sewer core", "detected", "not detected")), 
    values = factor(values, levels = c("not detected","detected", "Sewer core", "Gut bacteria, Disappearing")), 
    ) %>% 
  arrange(values) %>% 
  reframe(yintercept = cumsum(merge_abundances), values) 

plot <- plot_df %>%
  ggplot(aes(x = names, y = merge_abundances, fill = values)) +
  ...
  #scale_x_discrete(expand = c(0,0)) +
  facet_nested(~ System+SampleContent2) +
  labs(x = "Category", 
       fill = "Fill Color") +
  geom_hline(data = line_data, aes(yintercept = yintercept), color = "black", linewidth  =0.5,
               linetype = "solid") +  # Specific lines for each facet +
  geom_hline(aes(yintercept = 0), color = "black", linewidth  =0.5,
               linetype = "solid") +  # Specific lines for each facet +
  geom_vline(aes(xintercept = 2.75), 
               linetype = "solid", color = "black", linewidth  =0.5) +
  geom_vline(aes(xintercept = 0.76), 
               linetype = "solid", color = "black", linewidth  =0.5) +
   geom_text(data = label_df_groups, aes(
    y = mean_abun_sum - merge_abundances*0.95,
    color = if_else(str_detect(values, "Gut") & str_detect(names, "Source"), "white", "black"),
    label = case_when(merge_abundances > 30 ~ gsub("\\s*\\(.*?\\)", "", as.character(values)),
                      merge_abundances > 5.5 ~ gsub(".*?\\((.*?)\\).*", "\\1", as.character(values)),
                      .default = NA)), 
    angle = 90, hjust = 0, size = 4) +
  geom_label(data = label_df_numbers, aes(
    y = mean_abun_sum - merge_abundances*0.5,
    color = case_when(str_detect(values, "Gut") & names == 1 ~"white", 
                      str_detect(values, "isappearing")~"white", 
                      str_detect(values, "urviving")~"white", 
                      .default = "black"),
    label = if_else(merge_abundances > 4 | 
                       (str_detect(values, "growing") & merge_abundances > 0.5), paste0(as.character(round(merge_abundances, 1)), "%"), NA)), angle = 0, hjust = 0.5, vjust = 0.5, label.padding = unit(0.1, "lines")) +
  theme(legend.text = element_markdown(size = 16), 
          panel.background = element_rect(fill = "white"),
          axis.ticks.y = element_line(linewidth = unit(0.5, "lines"), color  = "black"),
        axis.title.y = element_text(size = 16),
        ...
        legend.position = "bottom",
        axis.text.y = element_text(size = 14, color  = "black"), 
        axis.text.x =  element_text(size = 14, angle  = 90, color  = "black", vjust = 0.5), #element_blank(), #element_markdown(angle = 90, size = 12),
        panel.spacing=unit(1,"mm"),
        axis.ticks.x = element_line(linewidth = unit(0.5, "lines"), color  = "black"),        
        legend.title = element_blank(), 
        strip.background.x = element_rect(fill = "grey90"),
        strip.background.y = element_blank()
        ) +
...
first_legend_plot <- 
  plot_df %>%
  filter(names == "Source") %>% 
  ggplot(., aes(x = merge_abundances, y = names, fill = values)) +
  geom_bar(stat = "identity", show.legend = TRUE) +  # Show the second legend
  scale_fill_manual(values = col2[17:21]) +
  theme_void() +  # Remove axes
  theme(legend.position = "bottom", legend.text = element_text(size = 16), legend.title = element_text(size = 16,  margin = margin(r = unit(15,"mm")))) +  # Position of the second legend
  labs(fill = "Source")  # Label for the second legend

second_legend_plot <- 
  plot_df %>%
  filter(names != "Source") %>% 
  mutate(values = (str_extract(values, "(?<=;).*")),
         values = factor(values, levels = c("growing", "surviving/varible", "disappearing", "unknown"),
                         labels = c("Growing", "Surviving/Varible", "Disappearing", "Unknown"))) %>% 
  ggplot(., aes(x = merge_abundances, y = names, fill = values)) +
  geom_bar(stat = "identity", show.legend = TRUE) +  # Show the second legend
  scale_fill_manual(values = rev(col2[1:4])) +
  theme_void() +  # Remove axes
  theme(legend.position = "bottom", legend.text = element_text(size = 16), legend.title = element_text(size = 16,  margin = margin(r = unit(15,"mm")))) +  # Position of the second legend
  labs(fill = "Growth\nfate in AS")  # Label for the second legend

x <- get_legend(second_legend_plot)
second_legend <- as_ggplot(x)
x <- get_legend(first_legend_plot)
first_legend <- as_ggplot(x)
...
data <- 
  ...
  summarise(mean_abun = mean(sum_growth), 
            ...
            .groups = "drop") %>% 
  group_by(SampleContent2) %>% 
  mutate(sum_mean_n_species = sum(mean_n_species), 
         p_mean_n_species = mean_n_species / sum_mean_n_species *100) %>% 
  ungroup()
...
df <- data %>%
  ...
  mutate(
    species_best_core = sum(mean_n_species), 
    p_species_best_core = sum(p_mean_n_species), 
    growth_AS = paste0(best_core_group, ";", growth_AS)
  ) %>%
  ungroup() %>% 
  pivot_longer(cols = c(best_core_group, growth_AS), 
               names_to = "names", values_to = "values") %>% 
  mutate(
    merge_p_species = if_else(names == "best_core_group", p_species_best_core, p_mean_n_species),
    merge_species = if_else(names == "best_core_group", species_best_core, mean_n_species),
    ) %>%
  select(-mean_abun, -mean_n_species, -species_best_core,
         -sum_mean_n_species,-p_mean_n_species, -p_species_best_core
         ) %>% distinct()

# blue_gradient <- c("#A7C7E7", "#7FAEDC", , "#2F7CC6")
# col2 = c(col, col, col, col, "#A7C7E7", "#D6604D", "grey90", "grey60")
# col2 = c(col, col, col, col,"#5795D1", "#A7C7E7",  "grey90", "grey60")
...
label_df_groups <- 
  ...
  group_by(SampleContent2, System) %>%
   mutate(
    mean_abun_sum = cumsum(merge_p_species)) 
...
line_data <- plot_df %>%
  ...
  arrange(values) %>% 
  reframe(yintercept = cumsum(merge_p_species), values) 

plot2 <- plot_df %>% 
  ggplot(aes(x = names, y = merge_p_species, fill = values)) +
  geom_bar(aes(width = width),
    stat = "identity",
    #color = "black",
    position="stack"
  ...
  #scale_x_discrete(expand = c(0,0)) +
  facet_nested(~ System+SampleContent2) +
  labs(x = "Category", y = "No. of species (transformed) [%]", fill = "Fill Color") +
  geom_hline(data = line_data, aes(yintercept = yintercept), color = "black", linewidth  =0.5,
               linetype = "solid") +  # Specific lines for each facet +
  geom_hline(aes(yintercept = 0), color = "black", linewidth  =0.5,
               linetype = "solid") +  # Specific lines for each facet +
  ...
  geom_vline(aes(xintercept = 0.76), 
               linetype = "solid", color = "black", linewidth  =0.5) +
   geom_text(data = label_df_groups, aes(
    y = mean_abun_sum - merge_p_species*0.95,
    color = if_else((str_detect(values, "Gut") & str_detect(names, "Source")), "white", "black"),
    label = case_when(merge_p_species > 25 & !str_detect(values, "sewer")~ gsub("\\s*\\(.*?\\)", "", as.character(values)),
                      merge_p_species > 43 & str_detect(values, "sewer") ~ gsub("\\s*\\(.*?\\)", "", as.character(values)),
                      merge_p_species > 5 ~ gsub(".*?\\((.*?)\\).*", "\\1", as.character(values)),
                      .default = NA)), 
    angle = 90, hjust = 0) +
  geom_label(data = label_df_numbers, aes(
    y = mean_abun_sum - merge_p_species*0.5,
    color = case_when(str_detect(values, "Gut") & names == 1 ~ "white", 
                      str_detect(values, "Gut bacteria, Disappearing;unknown")~"black",  
                      str_detect(values, "isappearing")~"white", 
                      str_detect(values, "urviving")~"white", 
                      .default = "black"),
    label = if_else(merge_p_species > 2.0 | 
                       (str_detect(values, "growing") & merge_p_species > 2), paste0(as.character(round(merge_species, 0))), NA)), 
    angle = 0, hjust = 0.5, vjust = 0.5, label.padding = unit(0.1, "lines"), size = 3.5) +
   theme(legend.text = element_markdown(size = 16), 
          panel.background = element_rect(fill = "white"),
          axis.ticks.y = element_line(linewidth = unit(0.5, "lines"), color  = "black"),
    ...
    scale_y_continuous(expand = c(0.0,0), breaks = c(0,25,50,75,100), labels = c("0%","25%","50%","75%","100%")) +
    scale_x_discrete(expand = c(0,0)) 
...
plot_legend <- first_legend + second_legend + plot_layout(nrow = 2) 
plot_name = "F5_stacked_bar_best_core_LEGEND"
ggsave(plot = plot_legend, paste0(OutputPath, "plots/1_results/", plot_name, ".png"), 
       width = 14, height = 1.5)
ggsave(plot = plot_legend, paste0(OutputPath, "plots/1_results/", plot_name, ".pdf"), limitsize = F,  useDingbats=FALSE,  
       width = 14, height = 1.5)
knitr::include_graphics(paste0(OutputPath, "plots/1_results/", plot_name, ".png"))
...
# Use annotation 
plot_merged <- plot + plot2 + plot_annotation(tag_levels = "A") &
theme(legend.position = "none", plot.tag = element_text(size = 30)) 

plot_name = "F5_stacked_bar_best_core"
ggsave(plot = plot_merged, paste0(OutputPath, "plots/1_results/", plot_name, ".png"), 
       width = 14.5, height = 7.8)
ggsave(plot = plot_merged, paste0(OutputPath, "plots/1_results/", plot_name, ".pdf"), limitsize = F,  useDingbats=FALSE,  
       width = 14, height = 7)
knitr::include_graphics(paste0(OutputPath, "plots/1_results/", plot_name, ".png"))



plot_merged + plot_legend + plot_layout(nrow = 2) 
...
# Read both pages from the PDF with very high resolution
images <- image_read_pdf("\Users\HD95LP\OneDrive - Aalborg Universitet\AAU\Publications\Sewer_Microbiome/Binder1.pdf", density = 2400)

images <- image_read_pdf("C:\\Users\\HD95LP\\OneDrive - Aalborg Universitet\\AAU\\Publications\\Sewer_Microbiome\\Binder1.pdf", density = 2400)


# Combine the two pages vertically
combined <- image_append(images, stack = TRUE)

# Save as a new single-page PDF
image_write(combined, path = "C:/Users/HD95LP/OneDrive - Aalborg Universitet/AAU/Publications/Sewer_Microbiome/Binder1_combined.pdf", format = "pdf")




# Read both pages from a single 2-page PDF
# OR replace with two separate files like: image_read_pdf("figure.pdf")
images <- image_read_pdf("C:\Users\HD95LP\OneDrive - Aalborg Universitet\AAU\Publications\Sewer_Microbiome\Binder1.pdf", density = 2400)

# Combine the two pages vertically (figure on top, legend below)
combined <- image_append(images, stack = TRUE)

# Write to a new single-page PDF
image_write(combined, path = "combined_page.pdf", format = "pdf")
...
## Growth groups per core group 

### Subplots {.tabset}

```{r}

all <- growing_dying_bar_plot(
  data_long %>% 
    filter(!SampleSite %in% c("AalborgWest_before")) %>% 
    mutate(
  ...
  include = c("Biofilm (g.)", "Sediment","Biofilm (e.p.)", "SWW", "IWW", "AS"),
  # gut_bacteria_as_group = F, 
  abun_limit = 4,
  #gut_df = gut_OTU
  ) +
   labs(title = "All") +
...
plot_name = "growth_of_all"
# ggsave(plot = all, paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"), 
#        width = 10, height = 5)
# knitr::include_graphics(paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"))
...
strict <- growing_dying_bar_plot(
  ...
  abun_limit = 4#,
   ...
   labs(title = "Strict unified core") +
   theme(plot.title = element_text(size = 18))


plot_name = "growth_of_the_strict_core"
ggsave(plot = strict, paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"), 
       width = 10, height = 5)
knitr::include_graphics(paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"))
...
general <- growing_dying_bar_plot(
  ...
  abun_limit = 4,
   ...
   labs(title = "General unified  core ") +
     theme(plot.title = element_text(size = 18))


plot_name = "growth_of_the_general_core"
ggsave(plot = general, paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"), 
       width = 10, height = 5)
knitr::include_graphics(paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"))
...
loose <- growing_dying_bar_plot(
  ...
  labs(title = "Loose unified core ") +
 theme(plot.title = element_text(size = 18))



plot_name = "growth_of_the_loose_core"
ggsave(plot = loose, paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"), 
       width = 10, height = 5)
knitr::include_graphics(paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"))
...
detected <- growing_dying_bar_plot(
  ...
  abun_limit = 4,
    ...
    labs(title = "Detected in the sewer microbiome") +
    theme(plot.title = element_text(size = 18))
...
ggsave(plot = detected, paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"), 
       width = 12, height = 7)
knitr::include_graphics(paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"))
...
unknown <- growing_dying_bar_plot(
  ...
  theme(plot.title = element_text(size = 18), 
        legend.text = element_text(size = 16),
        legend.title = element_text(size = 16)) +
  guides(fill = guide_legend(title = "Growth in AS")) +
  labs(title = "Unknown source")
...
### S10 Merge growth plots 

```{r}


legend <- get_legend(
  unknown + theme(legend.position = "right", text = element_text(size = 22)) + 
    labs(title = "Growth in AS")
)


# Adjusting the layout to center the first plot with specific width proportions
first_row <- (plot_spacer() | all + theme(legend.position = "none")  | legend) + 
  plot_layout(widths = c(0.1, 0.6, 0.2), heights = 2) # Set specific widths for the first row

# Adjusting the layout using patchwork
combined_plot <- 
  (first_row) / # First row: full-width plot
  (strict + theme(legend.position = "none") |  
     general + theme(axis.title.y = element_blank(), legend.position = "none")) / # Second row: two plots
  ( loose + theme(legend.position = "none") | 
      detected + theme(axis.title.y = element_blank(), legend.position = "none")) / # Third row: one plot, one empty space
  ( unknown + theme(legend.position = "none") | 
      gut + theme(axis.title.y = element_blank(), legend.position = "none"))  # Third row: one plot, one empty space
  

combined_plot <- combined_plot + plot_layout(heights = c(1.5, 1, 1, 1))

combined_plot
plot_name = "S10_Growth_fate_super_combined"
ggsave(plot = combined_plot, paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".png"), 
       width = 14, height = 17)
ggsave(plot = combined_plot, paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".pdf"), limitsize = F,  useDingbats=FALSE,  
       width = 16, height = 17)
...
IWW_AS_no_gut <- growing_dying_bar_plot(
  data_long %>% 
    filter(
      !SampleSite %in% c("AalborgWest_before")) %>% 
      mutate(
...
plot_name = "IWW_AS_no_gut"
ggsave(plot = IWW_AS_no_gut, paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"), 
       width = 4, height = 6)
knitr::include_graphics(paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"))
...
IWW_AS_with_gut <- growing_dying_bar_plot(
  data_long %>% 
    filter(
      !SampleSite %in% c("AalborgWest_before")) %>% 
      mutate(
        ...
        #                              filter(core_type == "strict core")))
                       ...
                       include = c("IWW", "AS"), 
                       gut_bacteria_as_group = T, abun_limit = 3.5,
                       gut_df = gut_OTU) 
# +
#    labs(title = "Strict  core ")


plot_name = "IWW_AS_with_gut"
ggsave(plot = IWW_AS_with_gut, paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"), 
       width = 6, height = 6)
knitr::include_graphics(paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"))
...
IWW_AS_with_gut <- growing_dying_bar_plot(
  data_long %>% 
      mutate(
        ...
        select(OTU, rel_abun_ASV, Species, rel_abun_species) %>% 
        filter(Species %in% unlist(core_species_all %>% 
                                      filter(core_type %in%  c("strict core", "general core", "loose core")))
               )
                         )), 
                       include = c("Biofilm (g.)", "Sediment", "Biofilm (e.p.)","SWW","IWW", "AS"),
                       gut_bacteria_as_group = F, abun_limit = 3.5,
                       gut_df = gut_OTU, make_label = F)
# + 
#   theme(legend.position = "none")
# +
#    labs(title = "Strict  core ")
  ...
  data_long %>% 
      mutate(
        ...
        samples = map(samples, ~ ungroup(.) %>% 
                        select(OTU, rel_abun_ASV, Species, rel_abun_species) %>% 
                        filter(Species %in% unlist(core_species_all %>% 
                                                     filter(core_type %in%  c("strict core",
                                                                              "general core", "loose core"))))
                         ),
...
bins <- c(0,0.0000000000000000001,0.01, 0.1, 1, 5, 10, Inf)
colors <- c("grey90","white","#FFF5EB", "#F5C9B0", "#EC9E75", "#E2733B", "#D94801")
fill_legend_label = c("0","0-0.01", "0.01-0.1", "0.1-1", "1-5", "5-10", ">10")
...
plot_df <- data_long %>% #sample_n(20) %>% 
   ...
   mutate(samples = 
    ...
    unnest(samples) %>%
  mutate(SampleSite = SampleContent2) %>%  #### MERGE SAMPLESITES
  group_by(SampleContent2, SampleContent3, SampleSite, lab, SampleID) %>% 
  reframe(
    sum = sum(rel_abun_genus), 
    n_genus = n(),
    Genus = if_else(lab == "Remaining", "", Genus)
    ) %>% 
  group_by(SampleContent2, SampleContent3,SampleSite, lab, Genus) %>% 
  summarise(
      mean = mean(sum), 
      mean_n_genus = mean(n_genus),
      .groups = "drop") %>% 
...
metabolic <- MIDAS_metabolic_df %>% 
  ...
  filter(!is.na(meta)) %>% 
  distinct() %>% 
  mutate(value = if_else(str_detect(Genus, "g__Azonexus") & meta == "Sulfate red.", "negative", value))
...
data_long$SampleContent3
# Make the heatmap 
plot <- 
  # HEATMAP
  new_plot_df %>% 
  #left_join(data_long %>% distinct(SampleContent2, SampleContent3) %>% 
  mutate(SampleContent3 = 
           str_replace(SampleContent3, " ", "\n"), 
                     SampleContent3 =  factor(SampleContent3, 
      levels = c("Biofilm\n(g.)",  "Sediment","Biofilm\n(e.p.)", 
                 "SWW\n(g.)","SWW\n(e.p.)", "IWW", "AS"))) %>% 
      #by = join_by(SampleContent2), relationship = "many-to-many"
             # ) %>% 
  mutate(Configuration = if_else(SampleContent2 %in% c("AS", "IWW"), "WWTP", "Sewer system")) %>% 
  ggplot(aes(x = SampleSite, y = lab, fill = as.factor(bin))) + 
  geom_tile() + 
  ggh4x::facet_nested(core_in~Configuration+SampleContent3, scales = "free", space = "free") +
  labs(fill="Relative abundance [%]") +
  scale_fill_manual(values = colors, na.value = "grey90",
                    breaks = c(1,2,3,4,5,6,7), 
                    labels = fill_legend_label) +
  scale_x_discrete(expand = c(0,0)) + 
...
plot_name = "F6_heatmap_core_genera_sewer_solids_after_metadata_update"
ggsave(plot = plot, paste0(OutputPath, "plots/1_results/", plot_name, ".png"), width = 14.5, height = 17)
ggsave(plot = plot, paste0(OutputPath, "plots/1_results/", plot_name, ".pdf"), limitsize = F,  useDingbats=FALSE, 
       width = 14.3, height = 17)
...
## Figure S7: HM of gut species inclunding genus name
...
top_each_samplecontent <- data_long %>%
  ...
  mutate(
    ...
    unnest(samples) %>% 
  group_by(Species, SampleEnvironment) %>% 
  summarise(mean = mean(rel_abun_species), .groups = "drop") %>% 
  #group_by(SampleEnvironment) %>% 
  slice_max(order_by = mean, n = 30) %>% ungroup() %>% 
  select(-mean)
...
plot_df <- data_long %>% #sample_n(20) %>% 
   ...
   mutate(
    ...
    unnest(samples) %>%
  #mutate(SampleSite = SampleContent2) %>%  #### MERGE SAMPLESITES
  group_by(SampleContent3, SampleSite, lab, SampleID) %>% 
  reframe(
    sum = sum(rel_abun_species), 
    n_species = n(),
    Species = if_else(lab == "Remaining", "", Species)
    ) %>% 
  group_by(SampleContent3, SampleSite, lab, Species) %>% 
  summarise(
      mean = mean(sum), 
      mean_n_species = mean(n_species),
      .groups = "drop") %>% 
...
new_plot_df <- rbind(new_rows, plot_df) %>% #print(n = 100) %>%
  ...
  mutate(
    ...
    lab = fct_recode(lab, "Remain. species"=" "),
        mean = ifelse(lab == "Remain. species", NA, mean),
    bin = cut(mean, breaks = bins, labels = FALSE, right = FALSE),
    #SampleContent3 = fct_recode(SampleContent3,"B.(e.p.)"="Biofilm (e.p.)"),
    SampleContent3 = fct_recode(SampleContent3,"Biofilm\n(e.p.)"="Biofilm (e.p.)")
    )
...
plot <- 
  # HEATMAP
  new_plot_df %>% 
  mutate(Configuration = if_else(SampleContent3 %in% c("AS", "IWW"), "WWTP", "Sewer system"),
         SampleContent3 = 
           factor(SampleContent3, 
            levels = c("Biofilm (g.)", "Sediment", "Biofilm\n(e.p.)", "SWW\n(g.)","SWW\n(e.p.)" ,"IWW","AS")),#,
           # labels = c("Biofilm (g.)", "Sediment", "Biofilm (e.p.)", "SWW (g.)","SWW\n(e.p.)" ,"IWW","AS")
         # SampleEnvironment_obs = ifelse(is.na(SampleEnvironment_obs), " ",
         #                                SampleEnvironment_obs),
         # SampleEnvironment_obs = factor(SampleEnvironment_obs,
  ...
  ggplot(aes(x = SampleSite, y = lab, fill = as.factor(bin))) + 
  geom_tile() + 
  ggh4x::facet_nested(x~Configuration+SampleContent3, scales = "free", space = "free") +
  labs(fill="Relative abundance [%]") +
  scale_fill_manual(values = colors, na.value = "grey90",
                    breaks = c(1,2,3,4,5,6,7), 
...
plot_name = "S7_heatmap_top25_gut_species_in_SWW"
ggsave(plot = plot, paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".png"), width = 13, height = 9.5)
ggsave(plot = plot, paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".pdf"), limitsize = F, useDingbats=FALSE, 
       width = 13, height = 9.5)
...
## Figure S13: Heatmap of functional guilds 
...
plot <- 
  ...
  theme(axis.text.y = element_markdown(size = 12),
        legend.text = element_text(size = 14, color = "black", 
                                   margin = margin(l = unit(5,"mm"))),
        legend.title = element_text(vjust = 0.8, size = 16, 
                                    color="grey10", 
                                    margin = margin(r = unit(15,"mm"))),
panel.grid = element_blank(),
        strip.text.x = element_text(size = 15.8),
...
plot_name = "S13_heatmap_functinal_guilds"
ggsave(plot = plot, paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".png"), width = 12.2, height = 10.5)
ggsave(plot = plot, paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".pdf"), limitsize = F, useDingbats=FALSE, width = 12.2, height = 10.5)
knitr::include_graphics(paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".png"))
...
## Figure S12: HM of the detected and not detected bacteria (growing)
...
bins <- c(0,0.0000000000000000001,0.001, 0.005, 0.01, 0.1, 1, Inf)
colors <- c("grey90","white","#FFF5EB", "#F5C9B0", "#EC9E75", "#E2733B", "#D94801")
fill_legend_label = c("0","0-0.001", "0.001-0.005", "0.005-0.01", "0.01-0.1", "0.1-1", ">1")
...
top_each_samplecontent <- data_long %>%
    # filter(SampleContent2 %in%
    #          #c("AS")) %>%
    #           c("AS","Biofilm (g.)", "Sediment", "Biofilm (e.p.)")) %>% 
  filter(SampleSite != "AalborgWest_before") %>% 
  mutate(
    samples = 
             map(.x = samples, ~
                   ungroup(.) %>% 
                   filter(Species %in% 
                            (best_core_group %>% filter(best_core_group == "detected") %>% pull(Species))) %>% 
                   filter(OTU %in% (growth_df %>% filter(growth_AS == "growing") %>% pull(OTU))) %>%         
                   #filter(OTU %in% unlist(gut_OTU$OTU)) %>% 
                   distinct(Species, rel_abun_species)  %>% 
                   filter(!str_detect(Species, "unclas"))
                 )
  ...
  summarise(mean = mean(rel_abun_species), .groups = "drop") %>% 
  group_by(SampleContent2, SampleEnvironment) %>% 
  slice_max(order_by = mean, n = 8) %>% ungroup() %>% 
  select(-mean)
...
plot_df <- data_long %>% #sample_n(20) %>% 
   ...
   mutate(
     ...
     SampleSite2 = substr(SampleSite, 1, 2),
     SampleSite = if_else(System == "Sewer system", 
                          paste0(SampleSite, ";", format(SampleDate,format="%d/%m/%y")), 
                          SampleSite)
     ) %>% #pull(SampleSite)
    unnest(samples) %>%
  #mutate(SampleSite = SampleContent2) %>%  #### MERGE SAMPLESITES
  group_by(SampleContent2, SampleSite, lab, SampleID, SampleSite2) %>% 
  reframe(
    sum = sum(rel_abun_species), 
    n_species = n(),
    Species = if_else(lab == "Remaining", "", Species)
    ) %>% 
  group_by(SampleContent2, SampleSite, lab, Species, SampleSite2) %>% 
  summarise(
      mean = mean(sum), 
      mean_n_species = mean(n_species),
      .groups = "drop") %>% 
...
new_plot_df <- 
  plot_df %>% 
  #rbind(new_rows, plot_df) %>% #print(n = 100) %>%
  #full_join(top_each_samplecontent_groups, by = join_by(Species)) %>%
  arrange(#SampleEnvironment_obs, 
          mean_across_all) %>% 
  mutate(
    lab = str_replace(lab, "Allorhizobium-Neorhizobium-Pararhizobium-Rhizobium","Allorhizobium-Neo."),
    lab = fct_inorder(lab),
    lab = fct_relevel(lab, "Remaining", after = 0),
    lab = fct_relevel(lab, " ", after = 0),
    lab = fct_recode(lab,"Remain. abundance"="Remaining"),
...
plot_detected <- 
  ...
  ggplot(aes(x = SampleSite, y = lab, fill = as.factor(bin))) + 
  geom_tile() + 
  ggh4x::facet_nested(x~Configuration+SampleContent2+SampleSite2, scales = "free"#, space = "free"
                      ) +
  labs(fill="Relative abundance [%]") +
  scale_fill_manual(values = colors, na.value = "grey90",
                    breaks = c(1,2,3,4,5,6,7), 
                    labels = fill_legend_label) +
  scale_x_discrete(expand = c(0,0)) + 
  geom_text(aes(label = case_when(mean >= 0.1 & Configuration == "WWTP"~ paste0(round(mean, 1)),
                                  .default = " " ))) +
  # geom_text(aes(label = case_when(mean >= 0.1 & lab != "Remain. abundance" &  lab != "Remain. species" ~
  #                                 paste0(round(mean, 1)),
  #                                 mean < 0.1 & lab != "Remain. abundance" &  lab != "Remain. species" ~ "",
  ...
  theme(axis.text.y = element_markdown(size = 12, color = "black"),
        ...
        strip.background = element_rect(fill = "grey90"),
        #strip.text.y = element_text(angle = 0, size = 16),
        #axis.text.x = element_text(size = 10, hjust = 1, vjust = 1, angle = 45,color = "black"),
  ...
  mutate(
    samples = 
             map(.x = samples, ~
                   ungroup(.) %>% 
                   filter(Species %in% 
                            (best_core_group %>% filter(best_core_group == "not detected") %>% pull(Species))) %>% 
                   filter(OTU %in% (growth_df %>% filter(growth_AS == "growing") %>% pull(OTU))) %>%         
                   #filter(OTU %in% unlist(gut_OTU$OTU)) %>% 
                   distinct(Species, rel_abun_species)  %>% 
                   filter(!str_detect(Species, "unclas"))
...
plot <- plot_detected + #theme(axis.text.x = element_blank()) +
  plot_not_detected +
  plot_annotation(tag_levels = "A") +
...
plot_name = "S12_heatmap_top10_growing_sewer_not_detected_species_in_AS"
ggsave(plot = plot, paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".png"), 
        width = 20, height = 19)
ggsave(plot = plot, paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".pdf"), limitsize = F, useDingbats=FALSE, 
       width = 20, height = 19)
...
plot <- 
  ...
  theme(axis.text.y = element_markdown(size = 12),
        ...
        strip.text.y = element_blank(), #element_text(angle = 0, size = 16),
        strip.background = element_rect(fill = "grey90"),
        axis.text.x = element_blank(), #element_text(size = 12, hjust = 0.5, vjust = 0.5, angle = 0),
        legend.position = "bottom",
        axis.ticks.x = element_blank()
        #plot.background = element_rect(fill = "grey70")
        ) +
...
plot_name = "S12_heatmap_PAOs"
ggsave(plot = plot, paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"), width = 8, height = 3)
ggsave(plot = plot, paste0(OutputPath, "plots/3_presentations/", plot_name, ".pdf"), limitsize = F, useDingbats=FALSE, width = 8, height = 4)
knitr::include_graphics(paste0(OutputPath, "plots/3_presentations/", plot_name, ".png"))
...
box <- 
  ...
  geom_jitter(aes(color = SampleContent2_2), size = 1, alpha = 0.65, shape = 16,
              width = 0.2) +
  #geom_boxplot(outlier.shape = NA, alpha = 0.2) +
  ...
  theme_minimal() + 
  theme(strip.background = element_rect(fill = "gray90", linetype = 1, color = "gray70"),
        strip.text = element_text(size = 14), 
        ...
        strip.text.y = element_blank(), 
        panel.border = element_rect(color = "gray70", fill = "transparent"),
        #axis.title.x = element_blank(), 
        legend.title = element_blank(),
...
### Figure S16: The Bray-Curtis (BC) distance between IWW and sewer environments as a function of time since rain event rain (TSR) for IWW from Aalborg East and West WWTP. 
...
plot <- cat_data %>% 
  ...
  geom_jitter(aes(fill = SampleContent2_2, shape = SampleContent2_2)) +
  scale_color_manual(values = SampleContent2_colors) +
  scale_fill_manual(values = SampleContent2_colors) +
  ...
  geom_smooth(aes(color = SampleContent2_2),
    ...
    se = T) + 
  ggpmisc::stat_poly_line(fill = "grey90", show.legend = F) +
  ...
  guides(color = guide_legend(override.aes = list(size = 5), title = "Habitat"),
         fill = "none", 
         shape = guide_legend(override.aes = list(size = 5), title = "Habitat"),
         line = "none") +
  theme_bw() +
  theme(legend.position = "bottom", 
        legend.text = element_text(size = 14, color = "black", margin = margin(l = unit(5,"mm"))),
        legend.title = element_text(vjust = 0.8, size = 16, 
                                    color="grey10", 
                                    margin = margin(r = unit(15,"mm"))),
        #legend.title = element_blank(),
...
plot_name = "S16_linear_model_BC_IWW_VS_sewer_AAW"
ggsave(plot = plot, paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".png"), width = 10, height = 8)
ggsave(plot = plot, paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".pdf"), 
       limitsize = F, useDingbats=FALSE,
       width = 10, height = 8)
...
### F7B: Subplot IWW More or less than TSR >= 3 Days
...
plot_AAW <- cat_data %>% 
  ...
  theme(strip.background = element_rect(fill = "gray90", linetype = 1, color = "gray70"),
        ...
        axis.title = element_markdown(#fill = "green4", 
                                        size = 14, color="grey10", 
                                        margin = margin(t = unit(10, "mm"), l = unit(10, "mm"))
                                        ), 
        strip.text.y = element_blank(), 
        panel.border = element_rect(color = "gray70", fill = "transparent"),
        #axis.title.x = element_blank(), 
...
## Figure 7: Combined plot for article 

```{r}

plot <- box + plot_AAW + 
  plot_layout(nrow = 2) & plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size = 25))
plot_name = "F7_Rain_effect_boxplot_combined"
ggsave(plot = plot, paste0(OutputPath, "plots/1_results/", plot_name, ".png"), width = 8, height = 7.5)
ggsave(plot = plot, paste0(OutputPath, "plots/1_results/", plot_name, ".pdf"), limitsize = F, useDingbats=FALSE, width = 8, height = 7.5)
knitr::include_graphics(paste0(OutputPath, "plots/1_results/", plot_name, ".png"))
...
## Figure S14: Cumulative abundance of sewer and gut core for rainy non rainy samples
...
data_long_rain_BP <- data_long %>%
  ...
  #colors <- c("#A7C7E7","#D6604D", "#F4A582", "#FDDBC7", "grey90","grey60")
colors <- c("#1E3F66", "#41729F", "#76A5C9", "#B0D0E8",  "grey90","grey60")  
  ...
  barplot_relative_abundance <- 
    ...
    geom_label(aes(x = SampleContent5, y = newy-(values*0.5), 
                   color = if_else(str_detect(origin_groups, "Gut") | 
                                     str_detect(origin_groups, "trict"),
                                   "white", "black"),
                   label = label
    ), 
    #position = position_dodge(width = 0.3),
    ...
    theme(#legend.text = element_markdown(size = 14), 
        ...
        legend.text = element_text(size = 14, color = "black", 
                                   margin = margin(l = unit(5,"mm"))),
        legend.title = element_text(vjust = 0.5, size = 16, 
                                    color="grey10", 
                                    margin = margin(r = unit(15,"mm"))),        
        axis.title.x = element_blank(),
...
plot_name = "S14_Abundance_core_groups_wet_and_dry"
...
TSR <- DMI_rain_df %>% 
  ...
  geom_point(aes(y = TSR, color = TSR_col), show.legend = F) +
  ...
  theme(
    ...
    axis.text.y = element_text(size = 14, color="grey10"),
    axis.text.x = element_text(size = 14, color="grey10"),
    axis.title.y =element_text(size = 14,color="grey10"),
    axis.title.x = element_blank(),
...
plot_name = "S15_TSR"
ggsave(plot = TSR, paste0(OutputPath, "plots/2_supplementary_figures/", plot_name, ".png"), width = 9, height = 5)
...
gradient1 <- c("#4D4D4D", "#A6A6A6", "#005082", "#2A7AA0", "#72B2D9", "#BBD5E5")
