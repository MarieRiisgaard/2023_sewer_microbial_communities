...
metadata <- metadata_l %>% 
  filter(Old != "old" | is.na(Old) | SampleName == "biofilm_dry") %>%    # Remove samples from old homogenisation 
  mutate(
...
MIDAS_metabolic_df <- import_MiDAS_metabolic_metadata(path = paste0(WD, "data/bacterial_groups/MiDAS_metadata/"),file_name = "20240618_MiDAS_Metadata")
